...
const Lugares = () => {
  ...
  async function agregarLugar() {
    try {
      await firebase.agregarLugar(
        estacionamientoInfo.id,
        lugares.length + 1,
        false,
        "habilitado"
      );
      Swal(
        ...
        CEstacionamientos.LUGAR_AGREGADO_CORRECTAMENTE
      );
    } catch (error) {
      Toast(traducirError(error.code));
    }
  ...
  async function habilitarLugar(id) {
    try {
      ...
      Swal(
        CGeneral.OPERACION_COMPLETADA,
        CEstacionamientos.LUGAR_HABILITADO_CORRECTAMENTE
      );
    } catch (error) {
      Toast(traducirError(error.code));
    }
  ...
  async function deshabilitarLugar(id) {
    try {
      ...
      Swal(
        CGeneral.OPERACION_COMPLETADA,
        CEstacionamientos.LUGAR_DESHABILITADO_CORRECTAMENTE
      );
    } catch (error) {
      Toast(traducirError(error.code));
    }
  ...
  async function liberarLugar(id) {
    try {
      ...
      Swal(
        CGeneral.OPERACION_COMPLETADA,
        CEstacionamientos.LUGAR_LIBERADO_CORRECTAMENTE
      );
    } catch (error) {
      Toast(traducirError(error.code));
    }
  ...
  return !cargando ? (
    <>
      ...
      <Typography className={classes.titulo}>
        Administrar lugares de mi estacionamiento
      </Typography>
      &nbsp;
      <Alert className={classes.alerta} severity="info" variant="filled">
        En esta pantalla usted podrá administrar todos los lugares disponibles
        dentro de su playa de estacionamiento. Además podrá liberar aquellos
        lugares que se encuentren ocupados en caso de necesitarlo.
      </Alert>
      <FormHelperText style={{ marginLeft: "1rem" }}>
        Aclaración: en caso de equivocarse con la distribución de lugares,
        recargue la página
      </FormHelperText>
      <Fab
      ...
      &nbsp;
      <Typography className={classes.cantidad}>
      ...
      <Grid container>
        <Grid item lg={8} xs={12}>
          <Card className={classes.cartaLugares}>
            {lugares
              .slice((pagina - 1) * itemsPorPagina, pagina * itemsPorPagina)
              .map((lugar) => (
                <>
                  <CardActionArea>
                    <CardContent key={lugar.id}>
                      {lugar.ocupado ? (
                        <Typography className={classes.ocupado}>
                          Ocupado
                        </Typography>
                      ) : (
                        ""
            ...
            <Grid item lg={4} xs={12}>
              <Card className={classes.cartaLugares}>
                <CardActionArea>
                  <CardContent>
                    <GrillaLugares
                      estacionamiento={estacionamientoInfo.id}
                      cantidadLugares={lugares.length}
                      lugares={lugares}
